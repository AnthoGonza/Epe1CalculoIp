...
public class ResultadoActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //OBTENEIENDO PROMEDIO y VALIDACION NOTA SUPERIOR A 4.0
        ...
        //VAliDANDO EXIMIDO O NO
        if(promedio >= 5.5 && minimo.size()==0){
            mensaje += "\nEstado ramo: EXIMIDO";
        }else{
            mensaje += "\nEstado ramo: A EXAMEN\n-------";
            //MOSTRANDO SI HAY NOTAS SOLEMNES BAJO 4.0
            if(minimo.size()>0 ){
                mensaje += "\nNota(s) solemne menor a 4.0:";
                for(String vari[]:minimo){
                    mensaje += "\n" + vari[0] + " : " + vari[1];
                }
                mensaje += "\n-------";
            }
            preMin=cal.preApr(promedio);
            //OBTENIENDO NOTA MINIMO PRESENTACION
            mensaje += "\nNOTA PRESENTACION: " + preMin[0];
            //OBTENIENDO NOTA MINIMO PRESENTACION
            mensaje += "\nNOTA MINIMA DE APROBACION: " + preMin[1];
            //MOSTRANDO OPCION CALCULAR NOTA FINAL
